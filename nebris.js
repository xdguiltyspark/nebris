(function nebris() {
  ...
  async function fetchFadeTime() {
    ...
    // Using CSS variable "--fade-time" for transition time
  }
  ...
  async function onSongChange() {
    ...
    if (!config.useCustomColor) {
      ...
      if (!config.useCurrSongAsHome && Spicetify.Player.data.item.metadata.image_url) {
        ...
        imageUrl = localStorage.getItem("nebris:startupBg") || defImage;
      }
  ...
  function galaxyFade() {
    //"Borrowed"(stolen) from Galaxy theme 
  ...
  const defImage = "https://i.imgur.com/Wl2D0h0.png";
  let startImage = localStorage.getItem("nebris:startupBg") || defImage;
  ...
  const editObserver = new MutationObserver((mutation_list) => {
    for (const mutation of mutation_list) {
      if (mutation.addedNodes.length) {
        ...
        optButton.onclick = () => {
          localStorage.removeItem(`nebris:playlistBg:${uid}`);
        ...
        save.addEventListener("click", () => {
          if (srcInput.value) {
            localStorage.setItem(`nebris:playlistBg:${uid}`, srcInput.value);
          }
        });
      }
    }
  ...
  bannerInput.onchange = () => {
    ...
    reader.onload = (event) => {
      ...
      if (!uid) {
        try {
          localStorage.setItem("nebris:startupBg", result);
        } catch {
        ...
        try {
          localStorage.setItem(`nebris:playlistBg:${uid}`, result);
        } catch {
          Spicetify.showNotification("File too large");
          return;
        }
  ...
  const homeEdit = new Spicetify.Topbar.Button("Hazy Settings", "edit", () => {
    ...
    img.src = localStorage.getItem("nebris:startupBg") || defImage;
    ...
    saveButton.addEventListener("click", () => {
      ...
      localStorage.setItem("nebris:startupBg", startImage);
      ...
      for (const option of [...optionList.children]) {
        ...
        console.log(
          `nebris: ${option.getAttribute("name")} set to ${option
